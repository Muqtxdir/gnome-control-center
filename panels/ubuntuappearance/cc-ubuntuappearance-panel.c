...
#define UBUNTU_STANDARD_THEME "Yaru"
#define UBUNTU_LIGHT_THEME "Yaru-light"
  ...
  if (selected != NULL)
    {
      ...
      {
        gtk_theme = UBUNTU_STANDARD_THEME;
        gedit_theme = UBUNTU_STANDARD_THEME;
      }
      else if (selected_item == self->theme_light)
      {
        gtk_theme = UBUNTU_LIGHT_THEME;
        gedit_theme = UBUNTU_STANDARD_THEME;
      }
...
on_interface_settings_changed (CcUbuntuAppearancePanel *self)
{
  ...
  if (g_str_equal (cursor_theme, UBUNTU_STANDARD_THEME) && g_str_equal (icon_theme, UBUNTU_STANDARD_THEME))
    {
      if (g_strcmp0 (gtk_theme, UBUNTU_STANDARD_THEME) == 0)
        theme_item = self->theme_standard;
      else if (g_strcmp0 (gtk_theme, UBUNTU_LIGHT_THEME) == 0)
...
cc_ubuntuappearance_panel_class_init (CcUbuntuAppearancePanelClass *klass)
{
  ...
  gtk_widget_class_bind_template_child (widget_class, CcUbuntuAppearancePanel, theme_standard);
  gtk_widget_class_bind_template_child (widget_class, CcUbuntuAppearancePanel, theme_light);
  ...
  gtk_widget_class_bind_template_child (widget_class, CcUbuntuAppearancePanel, font_listbox);
...
cc_ubuntuappearance_panel_init (CcUbuntuAppearancePanel *self)
{
  ...
  gtk_list_box_set_header_func (self->font_listbox,
